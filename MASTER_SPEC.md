...
I lokal preview kopierer final-commit-adapteren først til en tempfil i finalkatalogen, flusher tempfilen, verifiserer hash og publiserer deretter filen eller katalogen med `MoveFileExW(MOVEFILE_WRITE_THROUGH)`. Den ferdige filen reåpnes og flushes før receipt. Før planforsegling må den immutable targetprofilen bevise `supports_file_flush`, `supports_write_through_move` og durabilitynivået `file_flush_and_write_through_move_confirmed`; eldre flush-only-profiler blokkeres og må måles på nytt. Førstegangspublisering kan derfor returnere `LOCAL_FILE_FLUSH_AND_WRITE_THROUGH_MOVE_CONFIRMED` eller `LOCAL_DIRECTORY_MARKER_FLUSH_AND_WRITE_THROUGH_MOVE_CONFIRMED`. Idempotent katalogreplay utfører ikke en ny move og beholder den lavere `LOCAL_DIRECTORY_MARKER_FLUSH_CONFIRMED_ENTRY_UNCONFIRMED`. Recoveryjournalens `FINAL_DURABLE`-fase persisterer adapterens eksakte claim og flagg. Adapterretur alene og den ikke-støttede `REPLACEFILE_WRITE_THROUGH`-verdien brukes aldri som durabilitybevis. Fysisk mediegaranti og writable SMB-evidens er fortsatt utenfor den lokale claimen.
...
Terminal lifecycle er monotont `DURABLE -> RECONCILED -> CLEANUP_ELIGIBLE -> CLEANED`. `RECONCILED` krever at alle operationer for run-target er terminale, og at segmentet enten har journalreferanser eller er erstattet av et senere korrekt hashkjedet segment. Targetdokumentet slettes bare under en fersk `MutationPermit`, etter reparse-/typekontroll og eksakt sammenligning av alle immutable segmentfelt. Første cleanup-pass fjerner dokumentet men beholder `CLEANUP_ELIGIBLE`; bare et senere pass som observerer at filen fortsatt er borte kan journalføre `CLEANED`. Executor fullfører ikke run-target før alle segmenter er `CLEANED`. Startup godtar derfor både et fortsatt eksisterende immutable dokument for en database-rad som allerede er `RECONCILED`/`CLEANUP_ELIGIBLE`, og et manglende dokument for `CLEANUP_ELIGIBLE`; executor fortsetter den avbrutte monotone cleanupen. Manglende targetbevis mens databasen fortsatt er `RECONCILED`, eller et targetdokument som dukker opp igjen etter `CLEANED`, blokkerer derimot videre mutasjon.
