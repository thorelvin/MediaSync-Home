...
| 0A.5 — Windows argv/pakking | blocked | `spikes/0a5_windows_packaging/`; `tests/spikes/0a5_windows_packaging/`; `artifacts/0a5/` | `GetSystemDirectoryW`/argv, minimal runtime og lokal Nuitka standalone exe-smoke bestått; signering/SDK-inventory og ren Windows-VM mangler | Klargjør signering/SDK-inventory og ren VM-smoke før ADR-028 kan anbefales |
