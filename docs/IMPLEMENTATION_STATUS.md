...
| 0A.5 — Windows argv/pakking | blocked | `spikes/0a5_windows_packaging/`; `tests/spikes/0a5_windows_packaging/`; `artifacts/0a5/` | `GetSystemDirectoryW`/argv, minimal runtime, lokal Nuitka exe-smoke, SDK-tool-inventory og release-signing-plan bestått; signeringssertifikat/signert release og ren Windows-VM mangler | Bruk `artifacts/0a5/release-signing-plan.json` når signeringssertifikat/-policy og ren VM er klare |
