...
| `ADR-028` | Reproduserbar Python/PySide6-pakkestrategi for Windows | `0A.5` | `BLOCKED` | Pakkestrategi kan ikke anbefales for release før signeringssertifikat/-policy og ren Windows-VM er tilgjengelig og smoke-testet; minimal PySide6/BLAKE3/Nuitka-runtime, lokal Nuitka standalone exe-smoke, SDK/signing-tool inventory og release-signing-plan er bevist. | `PENDING` |  |  |
