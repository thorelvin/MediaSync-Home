...
| 0A.5 Windows argv and packaging | Blocked | `spikes/0a5_windows_packaging/`, `artifacts/0a5/` | Signing/SDK inventory and clean Windows VM smoke |
...
| ADR-028 Windows packaging | `BLOCKED` | Minimal PySide6/BLAKE3/Nuitka runtime, packaging scripts, and a local Nuitka standalone exe smoke pass in repo-local venv; signing/SDK inventory and clean VM smoke are missing | Zip-only Python runtime, unsigned local exe, or alternate packager | Reversal is medium before dependencies lock, high after release tooling | Keep blocked until signed/release build inventory and clean VM smoke pass | Provide signing/SDK/VM or defer scope |
