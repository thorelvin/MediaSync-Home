...
| ADR-028 Windows packaging | `BLOCKED` | Minimal PySide6/BLAKE3/Nuitka runtime, packaging scripts, local Nuitka standalone exe smoke, SDK/signing-tool inventory, and release-signing-plan pass; signing certificate/signing operation and clean VM smoke are missing | Zip-only Python runtime, unsigned local exe, or alternate packager | Reversal is medium before dependencies lock, high after release tooling | Keep blocked until a signing policy/certificate and clean VM smoke pass | Use `artifacts/0a5/release-signing-plan.json` when signing policy/certificate and VM are ready, or defer scope |
