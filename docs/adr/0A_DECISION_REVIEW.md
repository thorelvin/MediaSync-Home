...
| 0A.5 Windows argv and packaging | Blocked | `spikes/0a5_windows_packaging/`, `artifacts/0a5/` | Signing certificate/signing operation and clean Windows VM smoke |
...
| ADR-028 Windows packaging | `BLOCKED` | Minimal PySide6/BLAKE3/Nuitka runtime, packaging scripts, local Nuitka standalone exe smoke, and SDK/signing-tool inventory pass; signing certificate/signing operation and clean VM smoke are missing | Zip-only Python runtime, unsigned local exe, or alternate packager | Reversal is medium before dependencies lock, high after release tooling | Keep blocked until a signing policy/certificate and clean VM smoke pass | Provide signing policy/certificate and VM, or defer scope |
...
| Local unsigned exe preview only | No signed release or clean-VM claim | Avoids false packaging promise; not a user-ready release | ADR-028 |
