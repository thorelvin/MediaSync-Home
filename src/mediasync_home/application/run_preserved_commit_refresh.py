...
MAX_RUN_TARGET_PRESERVED_COMMIT_REFRESH_SCAN = 1000


class RunTargetPreservedCommitRefreshOperationStore(RecoveryOperationStore, Protocol):
...
class RunTargetPreservedCommitRefreshOutcome:
    idle: bool
    refreshed: bool
    run_id: str
    run_target_id: str
...
def refresh_next_run_target_preserved_commit_intent_for_fresh_lease(
    *,
    permit: MutationPermit,
    recovery_operations: RunTargetPreservedCommitRefreshOperationStore,
    intent_segments: RecoveryIntentSegmentStore,
    process_instance_id: str,
    max_operations: int,
) -> RunTargetPreservedCommitRefreshOutcome:
    if not process_instance_id.strip():
        return _failed(
            permit=permit,
            operation=None,
            segment=None,
            validation_code="RUN_TARGET_PRESERVED_COMMIT_REFRESH_REQUIRES_PROCESS_INSTANCE",
            next_action="Bind preserved-commit refresh to the Engine Host process instance.",
        )
    if max_operations < 1:
        return _failed(
            permit=permit,
            operation=None,
            segment=None,
            validation_code="RUN_TARGET_PRESERVED_COMMIT_REFRESH_REQUIRES_POSITIVE_LIMIT",
            next_action="Retry preserved-commit refresh with a positive bounded limit.",
        )
    if max_operations > MAX_RUN_TARGET_PRESERVED_COMMIT_REFRESH_SCAN:
        return _failed(
            permit=permit,
            operation=None,
            segment=None,
            validation_code="RUN_TARGET_PRESERVED_COMMIT_REFRESH_LIMIT_TOO_LARGE",
            next_action="Retry preserved-commit refresh with a smaller bounded limit.",
        )

    operation = _next_stale_preserved_operation(
        permit=permit,
        recovery_operations=recovery_operations,
        max_operations=max_operations,
    )
    if operation is None:
        return RunTargetPreservedCommitRefreshOutcome(
            idle=True,
            refreshed=False,
            ...
            segment=None,
            validation_codes=(),
            next_action="No preserved replacement needs fresh lease intent refresh.",
        )
    if not _operation_static_binding_matches_permit(operation=operation, permit=permit):
        return _failed(
            ...
            operation=operation,
            segment=None,
            validation_code="RUN_TARGET_PRESERVED_COMMIT_REFRESH_PERMIT_MISMATCH",
            next_action="Reconcile recovery operation ownership before refreshing preserved commit intent.",
        )
    if operation.intent_segment_id is None or operation.intent_ordinal is None:
        return _failed(
            ...
            operation=operation,
            segment=None,
            validation_code="RUN_TARGET_PRESERVED_COMMIT_REFRESH_REQUIRES_EXISTING_INTENT",
            next_action="Recover or reconcile the existing commit intent segment before preserved replacement.",
        )
    ...
    if current_segment is None or not _segment_matches_operation(
        ...
        return _failed(
            ...
            operation=operation,
            segment=current_segment,
            validation_code="RUN_TARGET_PRESERVED_COMMIT_REFRESH_SEGMENT_MISMATCH",
            next_action="Recover or reconcile the existing commit intent segment before preserved replacement.",
        )
    ...
    if latest_segment is None:
        return _failed(
            ...
            operation=operation,
            segment=current_segment,
            validation_code="RUN_TARGET_PRESERVED_COMMIT_REFRESH_SEGMENT_CHAIN_MISSING",
            next_action="Recover or reconcile recovery intent segment history before preserved replacement.",
        )
    ...
    try:
        ...
        refreshed = recovery_operations.record_old_target_preserved_commit_intent_refreshed(
        ...
        return _failed(
            ...
            next_action=_error_next_action(
                exc,
                "Reload recovery intent state before retrying preserved-commit refresh.",
            ),
        )
    if refreshed is None:
        return _failed(
            ...
            operation=operation,
            segment=segment,
            validation_code="RUN_TARGET_PRESERVED_COMMIT_REFRESH_CONFLICT",
            next_action="Reload recovery operation state before retrying preserved-commit refresh.",
        )
    ...
    return RunTargetPreservedCommitRefreshOutcome(
        ...
        segment=published,
        validation_codes=(),
        next_action="Preserved replacement commit intent is refreshed for the fresh endpoint lease.",
    )


def _next_stale_preserved_operation(
    *,
    permit: MutationPermit,
    recovery_operations: RunTargetPreservedCommitRefreshOperationStore,
    max_operations: int,
