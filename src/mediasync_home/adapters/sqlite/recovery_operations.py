...
def _operation_from_row(row: sqlite3.Row | tuple[Any, ...]) -> RecoveryOperation:
    return RecoveryOperation(
        ...
        endpoint_generation=int(row[9]),
        owner_installation_id=str(row[10]),
        ownership_epoch=int(row[11]),
        lease_id=str(row[12]),
        lease_resource_key=str(row[13]),
        fencing_token=int(row[14]),
        phase=RecoveryOperationPhase(str(row[15])),
        source_relative_path=None if row[16] is None else str(row[16]),
        source_guard_kind=None if row[17] is None else str(row[17]),
        source_guard_evidence_hash=None if row[18] is None else str(row[18]),
        source_hash_evidence_kind=None if row[19] is None else str(row[19]),
        source_path_chain_hash=None if row[20] is None else str(row[20]),
        source_case_context_hash=None if row[21] is None else str(row[21]),
        staging_object_id=None if row[22] is None else str(row[22]),
        final_relative_path=str(row[23]),
        version_object_id=None if row[24] is None else str(row[24]),
        quarantine_object_id=None if row[25] is None else str(row[25]),
        intent_segment_id=None if row[26] is None else str(row[26]),
        intent_ordinal=None if row[27] is None else int(row[27]),
        target_precondition_kind=RecoveryTargetPreconditionKind(str(row[28])),
        expected_source_fingerprint_json=None if row[29] is None else str(row[29]),
        expected_target_fingerprint_json=None if row[30] is None else str(row[30]),
        expected_source_parent_identity_json=None if row[31] is None else str(row[31]),
        expected_target_parent_identity_json=None if row[32] is None else str(row[32]),
        expected_target_path_chain_hash=None if row[33] is None else str(row[33]),
        expected_staging_fingerprint_json=None if row[34] is None else str(row[34]),
        expected_final_fingerprint_json=None if row[35] is None else str(row[35]),
        observed_target_file_id=None if row[36] is None else str(row[36]),
        transfer_state=None if row[37] is None else str(row[37]),
        assurance_level=None if row[38] is None else str(row[38]),
        staging_durability_state=None if row[39] is None else str(row[39]),
