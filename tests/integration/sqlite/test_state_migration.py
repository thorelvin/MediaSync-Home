...
def test_fresh_state_initialization_commits_one_restartable_epoch(
    ...
    assert [
        ...
        (SqliteStore.CATALOG, 0, 56),
        (SqliteStore.RECOVERY, 0, 12),
    ]
...
def test_existing_state_upgrade_resumes_after_only_catalog_migrates(
    ...
    assert _schema_versions(layout) == (57, 12)
    ...
    assert [store.schema_version for store in backup.stores] == [56, 12]
    ...
    assert _schema_versions(layout) == (57, 13)
