...
def test_fresh_state_initialization_commits_one_restartable_epoch(tmp_path: Path) -> None:
    ...
    assert [
        ...
        (SqliteStore.CATALOG, 0, 36),
        (SqliteStore.RECOVERY, 0, 7),
    ]
...
def test_existing_state_upgrade_resumes_after_only_catalog_migrates(
    ...
    assert _schema_versions(layout) == (37, 7)
    ...
    assert [store.schema_version for store in backup.stores] == [36, 7]
    ...
    assert _schema_versions(layout) == (37, 8)
