...
def test_sqlite_engine_host_startup_reconciliation_reverifies_old_target_preserved(
    tmp_path: Path,
) -> None:
    catalog_database = tmp_path / "catalog.sqlite"
    recovery_database = tmp_path / "recovery.sqlite"
    ...
    with sqlite3.connect(catalog_database) as catalog_connection:
        ...
        try:
            ...
            _mark_run_target_executing(runs)
            _record_old_target_preserved_operation(
                recovery_operations,
                content_hash=content_hash,
            )
...
def _record_old_target_preserved_operation(
    store: SqliteRecoveryOperationStore,
    *,
    content_hash: str = "a" * 64,
) -> RecoveryOperation:
    operation = _record_commit_preconditions_operation(
        store,
        content_hash=content_hash,
    ...
    updated = store.record_operation_phase_transition(
        ...
        process_instance_id="host-a",
        operation_metadata=RecoveryOperationMetadata(version_object_id="version-a"),
    )
    assert updated is not None
    return updated


def _catalog_resume_operation(
    *,
    content_hash: str = "a" * 64,
    target_precondition_kind: RecoveryTargetPreconditionKind = RecoveryTargetPreconditionKind.ABSENT,
) -> RecoveryOperation:
