...
def test_engine_host_runtime_state_root_initializes_sqlite_and_persists_receipts(
    ...
    try:
        ...
        assert runtime.version_restore_store is not None
        assert runtime.version_restore_lease_authority is not None
        assert runtime.version_restore_filesystem_port is not None
        assert runtime.service.version_restore_request_store is not None
        assert runtime.service.version_restore_undo_request_store is not None
        assert (
...
def test_local_writable_runtime_creates_job_from_inline_gui_draft(
    ...
    try:
        ...
        catalog_cross_store = runtime.catalog_connection.execute(
            """
            SELECT handoff_type, state
            FROM store_handoffs
            WHERE entity_id = ? OR entity_id LIKE ?
            ORDER BY handoff_type
            """,
            (checked.started_run_id, f"{checked.started_run_id}:%"),
        ).fetchall()
        recovery_cross_store = runtime.recovery_connection.execute(
            """
            SELECT handoff_type, state
        ...
        assert catalog_cross_store == [
            ("OPERATION_CATALOG_RECORD", "COMPLETED"),
            ("RUN_START", "COMPLETED"),
        ]
        assert recovery_cross_store == catalog_cross_store
