...
def test_restore_next_run_target_preserved_old_target_cancels_quarantined_empty_directory() -> None:
    recovery_operations = _FakeRecoveryOperationStore((_directory_operation(),))
    restore_port = _FakeOldTargetRestorePort()
...
def _directory_operation(*, lease_id: str = "lease-a") -> RecoveryOperation:
    return replace(
        planned_recovery_operation(
            run_id="run-a",
            run_target_id="run-a-target-0000",
